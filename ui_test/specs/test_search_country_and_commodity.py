...
def test_search_country_and_commodity_by_keyword(browser):
    ...
    assert(browser.is_text_present('Showing 13 results for Coffee'))


def test_search_country_and_commodity_by_code(browser):
    search_country(browser, 'Brazil')
    search_commodity(browser, '2101129800')
    assert(browser.is_text_present('Export 2101.12.98 from Brazil to the United Kingdom'))
