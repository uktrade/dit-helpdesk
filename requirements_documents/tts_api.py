...
# 'producline_suffix', 'description', 'number_indents', 'goods_nomenclature_item_id', 'bti_url', 'formatted_description', 'description_plain',
# 'consigned_from', 'basic_duty_rate', 'meursing_code', 'section', 'chapter', 'declarable', 'import_measures', 'export_measures', 'footnotes',
# 'heading', 'ancestors', '_response_info'
...
class CommodityJson(object):
    ...
    def __repr__(self):
        return 'CommodityJson' #'CommodityJson for: '+self.commodity_code
    ...
    @property
    def code(self):
        return self.di['goods_nomenclature_item_id']  #, '0706100010')
    ...
    def get_import_measures(self, origin_country, vat=None, excise=None):
        ...
        #print('warning: measure_type filter disabled in get_import_measures()')
        # filter out irrelevant measures by checking measure_type_id

        measures = [
            obj for obj in measures if obj.type_id in TTS_MEASURE_TYPES
        ]

        #settings.TTS_MEASURE_TYPES
...
class ImportMeasureJson(object):
    ...
    #     self.commodity_code = kwargs['commodity_code']
    #     del kwargs['commodity_code']
    #     super(ImportMeasureJson, self).__init__(*args, **kwargs)

    def __repr__(self):
        return 'ImportMeasureJson %s %s' % (self.commodity_code, self.type_id)

    # ('effective_start_date', '2019-01-01T00:00:00.000Z')
    # effective_end_date
...
'''
('excise', False)
>>> [tup for tup in di2['import_measures'][0].items()][10]
('vat', True)
>>> [tup for tup in di2['import_measures'][0].items()][11]
('measure_type', {'id': 'VTZ', 'description': 'VAT zero rate'})
'''

# import_measures[0].keys()
# ['id', 'origin', 'effective_start_date', 'effective_end_date', 'import', 'measure_conditions', 'excluded_countries',
# 'footnotes', 'order_number', 'excise', 'vat', 'measure_type', 'duty_expression', 'geographical_area']


# todo: find measure_series_id which you can find with the measure_type_id in postgres DB dump
...
def get_commodity_data(commodity_id):
    assert len(commodity_id) == 10
    resp = requests.get(COMMODITY_URL % commodity_id)
    di = json.loads(resp.content.decode())
    return CommodityJson(di)
...
def do_stuff():
    for code in COMMODITY_CODES:
        data = get_commodity_data(code)
        for measure in data.get_import_measures():
            import pdb; pdb.set_trace()
            print(measure.order_number_definition_summary)
