...
const buildStylesForHelpdesk = () =>
  buildStylesForModernBrowsers(paths.styles.helpdesk);

const buildStylesForCMS = () => buildStylesForModernBrowsers(paths.styles.cms);

const buildStylesForOldIE = () => {
  return gulp
    .src(paths.styles.oldie)
    .pipe(
      sass({
        includePaths: "node_modules",
      }).on("error", sass.logError)
    )
    .pipe(postcss([autoprefixer, oldie]))
    .pipe(cssnano())
    .pipe(
      rename({
...
const bundle = (filename) => {
  var b = browserify({
    entries: ["./assets/javascript/" + filename + ".js"],
    debug: true,
...
const copyGOVUKFrontendAssets = () => {
  return gulp
    .src(paths.govukFrontendAssets.source)
...
const buildStyles = gulp.parallel(
  buildStylesForHelpdesk,
  buildStylesForCMS,
  buildStylesForOldIE
);
...
const watch = gulp.parallel(
  watchStyles,
  watchJavascripts,
  compileGovukFrontend,
  compileLocationAutocomplete,
  compileCMS
);
