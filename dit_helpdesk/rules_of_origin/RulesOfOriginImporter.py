...
CHAPTER_MATCHES = (
    ...
    (
        "^ex\xa0Chapter\xa0(?P<chapter_num>\d{1,2})$",
        "chapter_exclusion",
    ),  # ex\xa0Chapter 4
    ("^Ex Chapter (?P<chapter_num>\d{1,2})$", "chapter_exclusion"),  # Ex Chapter 4
    (
        "^(ex\xa0ex\d{4}\s)+(ex\xa0ex\d{4})$",
    ...
    (
        "ex(\xa0(?P<heading_num>\d{4})[,\s| and ]?)",
        "heading_num_list",
    ),  # ex 7107, ex 7109 and ex 7111
    (
        "ex(\xa0ex(?P<heading_num>\d{4})[,\s| and ]?)",
        "heading_num_list",
    ),  # ex 7107, ex 7109 and ex 7111
    ("ex\xa0\xa0(?P<heading_num>\d{2}\xa0\d{2})?", "heading_num_split"),  # ex  96 13
    (
        "ex\xa0\xa0(?P<heading_num>\d{2}\xa0\d{2})?",
        "heading_num_split_range",
    ),  # ex  96 13
    (
        "ex\xa0\xa0(?P<heading_num>\d{2}\xa0\d{2})[ and ]?",
        "heading_num_split_range",
...
class RulesOfOriginImporter:
    ...
    def instance_builder(self):
        ...
        if created:
            ...
            logger.debug(
                "{} instance already exists".format(rules_group._meta.model_name)
            )
        ...
        try:
            for country_code in self.rules_groups[self.working_group_name]:
                try:
                    ...
                    if created:
                        logger.debug(
                            "{0} instance created".format(group_member._meta.model_name)
                        )
                    else:
                        logger.debug(
                            "{0} instance already exists".format(
                                group_member._meta.model_name
                            )
                        )
                except ObjectDoesNotExist as exception:
                    logger.debug(
                        "country {0} does not exist {1}".format(
                            country_code, str(exception)
                        )
                    )
        except KeyError as exception:
            logger.debug("country_code not in rules groups {0}".format(exception.args))

        try:
            doc_url = self.rules_documents[self.working_group_name]
        except Exception as exception:
            logger.debug(
                "no {0} rules documents{1}".format(
                    self.working_group_name, exception.args
                )
            )
            doc_url = None
        ...
        if created:
            ...
            logger.debug(
                "{0} instance already exists".format(rules_document._meta.model_name)
            )
        ...
        if self.footnotes:
            for footnote in self.footnotes:
                ...
                if created:
                    logger.debug(
                        "{0} instance created".format(footnote._meta.model_name)
                    )
                else:
                    logger.debug(
                        "{0} instance already exists".format(footnote._meta.model_name)
                    )
        ...
        for rules_chapter in self.data:
            ...
            for level in self.data[rules_chapter]:
                ...
                for rule in self.data[rules_chapter][level]:
                    ...
                    try:
                        related_chapter = Chapter.objects.get(
                            chapter_code=rule["chapter_code"]
                        )
                    except ObjectDoesNotExist as odne:
                        logger.debug(
                            "no chapter objects {0} for chapter code {1}".format(
                                odne.args, rule["chapter_code"]
                            )
                        )
        ...
        for rule in rules:
            ...
            for item in rule["rows"]:
                ...
                if regex_match is not None:
                    ...
                    if (
                        level in self.data[rule["number"]].keys()
                        and len(self.data[rule["number"]][level]) > 0
                    ):
    ...
    def match_hierarchy_level(item_id):
        ...
        """
        matches = [
            (re.search(pattern, item_id[0]), category)
    ...
    def set_relationships(self, item, regex_match, chapter_code):
        ...
        if {"chapter_num"} <= set(match_results.keys()):
            chapter_code = self.get_hierarchy_code(
                regex_match.group("chapter_num"), "Chapter"
            )
            related_hierarchy_levels = self.get_related_hierarchy_levels(
                "Chapter", chapter_code, item, parent_ids_list
            )

        elif {"heading_num", "start_heading", "end_heading"} <= set(
            match_results.keys()
        ):
            heading_code = self.get_hierarchy_code(
                regex_match.group("heading_num").replace("\xa0", ""), "Heading"
            )

            start_heading_code = "{0}".format(regex_match.group("start_heading")).ljust(
                10, "0"
            )
            end_heading_code = "{0}".format(regex_match.group("end_heading")).ljust(
                10, "0"
            )

            code_range = [
                self.get_hierarchy_code(i, "Heading")
                for i in range(
            ...
            related_range = self.get_related_hierarchy_levels(
                "Heading",
                start_heading_code,
        ...
        elif {"start_heading", "end_heading"} <= set(match_results.keys()) and not {
            "heading_num"
        } <= set(match_results.keys()):
            ...
            for value in match_results.values():
                code_range.append(
                    self.get_hierarchy_code(value.replace("\xa0", ""), "Heading")
                )
            related_hierarchy_levels = self.get_related_hierarchy_levels(
                "Heading", code_range=code_range
            )
            ...
            for value in match_results.values():
                code_range.append(
                    self.get_hierarchy_code(value.replace("\xa0", ""), "SubHeading")
                )

            related_hierarchy_levels = self.get_related_hierarchy_levels(
                "SubHeading", code_range=code_range
            )
        else:
            ...
            if code_range:
                related_objects = list(
                    [
                        ...
                        for heading in Heading.objects.filter(
                            heading_code__in=code_range
                        )
                    ]
                )
