...
class GetCommodityCodeDataTestCase(TestCase):
    ...
    def test_get_commodity_code_data_test(self, mock_requests):
        ...
        mock_requests.get(f"{ROOT_URL}?q={code}", json=test_response)
...
class GetCommodityDataTestCase(TestCase):
    ...
    def test_commodity_has_direct_result(self, mock_requests):
        ...
        test_response = [
            self._get_test_commodity_response(
                hierarchy_codes[0], f"Desc for {hierarchy_codes[0]}"
            )
        ]
        mock_requests.get(self._get_url(hierarchy_codes[0]), json=test_response)
    ...
    def test_commodity_has_direct_ancestor_result(self, mock_requests):
        ...
        mock_requests.get(self._get_url(hierarchy_codes[0]), json=test_no_data_response)

        test_response = [
            self._get_test_commodity_response(
                hierarchy_codes[1], f"Desc for {hierarchy_codes[1]}"
            )
        ]
        mock_requests.get(self._get_url(hierarchy_codes[1]), json=test_response)
    ...
    def test_commodity_has_indirect_ancestor_result(self, mock_requests):
        ...
        mock_requests.get(self._get_url(hierarchy_codes[0]), json=test_no_data_response)
        mock_requests.get(self._get_url(hierarchy_codes[1]), json=test_no_data_response)

        test_response = [
            self._get_test_commodity_response(
                hierarchy_codes[2], f"Desc for {hierarchy_codes[2]}"
            )
        ]
        mock_requests.get(self._get_url(hierarchy_codes[2]), json=test_response)
    ...
    def test_commodity_has_no_results(self, mock_requests):
        ...
        mock_requests.get(self._get_url(hierarchy_codes[0]), json=test_no_data_response)
        mock_requests.get(self._get_url(hierarchy_codes[1]), json=test_no_data_response)
        mock_requests.get(self._get_url(hierarchy_codes[2]), json=test_no_data_response)
